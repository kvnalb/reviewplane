...
test('supports keyboard activation, hover targeting, and bounded popups', async ({ page }) => {
  ...
  await expect(page.getByRole('textbox', { name: 'Foreground color' })).toHaveValue('rgb(17, 24, 39)')
  await expect(page.getByRole('textbox', { name: 'Background color' })).toHaveValue('rgba(0, 0, 0, 0)')
  await expect(page.getByRole('textbox', { name: 'Font size' })).toHaveValue('36px')
...
test('previews four direct changes, supports middle undo, and resets to baseline', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Foreground color' }).fill('#be3227')
  await page.getByRole('textbox', { name: 'Background color' }).fill('#fff7ed')
  await page.getByRole('textbox', { name: 'Font size' }).fill('20px')
...
test('resolves a real pending mock WebMCP wait on Done', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Inspect', exact: true }).click()
  await page.getByRole('heading', { name: 'Review fixture' }).click()
  await page.getByRole('textbox', { name: 'Font size' }).fill('48px')
  await page.getByRole('button', { name: 'Add correction' }).click()
  await page.getByRole('button', { name: 'Done' }).click()
